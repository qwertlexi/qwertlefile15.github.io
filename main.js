(function () {
  ...
  // Bar expand is now handled by dEnd for mouse (avoids double-fire with drag)
  // Touch tap still needs explicit handling
  if (playerBar) playerBar.addEventListener("touchend", function(e){
    if (drag.moved) return; // was a drag, not a tap
  ...
  if (playerEl && playerBar) {
    ...
    function getViewportOffset() {
      // On iOS Safari, when the address bar is visible, window.innerWidth/Height
      // may differ from the visual viewport. Use visualViewport if available.
      if (window.visualViewport) {
        return { x: window.visualViewport.offsetLeft, y: window.visualViewport.offsetTop };
      }
      return { x: 0, y: 0 };
    }
    function dStart(cx,cy) {
      var r   = playerEl.getBoundingClientRect();
      var vpo = getViewportOffset();
      // For position:fixed, the element's CSS left/top are relative to the
      // layout viewport origin. On iOS Safari with a visible address bar,
      // getBoundingClientRect() is in visual viewport coords, so we subtract
      // the visual viewport offset to get layout-viewport-relative values.
      var snapLeft = r.left - vpo.x;
      var snapTop  = r.top  - vpo.y;
      playerEl.style.bottom    = "auto";
      playerEl.style.right     = "auto";
      playerEl.style.left      = snapLeft + "px";
      playerEl.style.top       = snapTop  + "px";
      playerEl.style.transform = "none";
      drag.on = true; drag.moved = false;
      drag.sx = cx; drag.sy = cy;
      drag.ox = snapLeft; drag.oy = snapTop;
    ...
    function dMove(cx,cy) {
      ...
      var nl = drag.ox + dx, nt = drag.oy + dy;
      ...
      playerEl.style.left = nl + "px";
      playerEl.style.top  = nt + "px";
    }
    function dEnd(e) {
      if (!drag.on) return;
      drag.on = false;
      playerEl.classList.remove("is-dragging");
      // If mouse barely moved, treat as click → expand
      if (!drag.moved && e && e.type === "mouseup") {
        if (expandBtn) expandBtn.click();
      }
      drag.moved = false;
    }
