(function () {
  ...
  function drawRain() {
    ...
    beatEnergy += (_energy - beatEnergy) * 0.14;
    var _delta = _energy - lastBeatEnergy;
    if (_delta > 0.12) {
      beatFlash = Math.min(1.0, beatFlash + _delta * 3.5); // accumulate on rapid beats
    }
    lastBeatEnergy = _energy;
    beatFlash *= 0.76; // faster decay = snappier pulse
    ...
    for (var i = 0; i < cols.length; i++) {
      ...
      for (var t = 0; t < tL; t++) {
        ...
        var fx = 0, fy = 0;
        if (mdist < PART_RADIUS && mdist > 0.5) {
  ...
  function initBeatAnalyser() {
    ...
    try {
      ...
      beatAnalyser.smoothingTimeConstant = 0.6;
  ...
  var CMD_HELP = [
    ...
    "whoami  ·  history  ·  lyrics  ·  capsule  ·  ping <url>",
  ...
  var ALL_CMDS = ["help","clear","rain","speed","theme","time","echo","matrix","cyber","fps","capsule","about","lyrics",
    ...
    "rain on","rain off","speed s","speed n","speed f","theme d","theme l","whoami","history"];
