(function () {
  ...
  if (volSlider) {
    /* iOS Safari ignores audio.volume changes unless audio was started by a
       user gesture on the element itself. We work around this by routing
       through a Web Audio API GainNode, which respects the OS volume and
       works on all platforms including iOS.
       Fallback: if AudioContext is unavailable, still set audio.volume. */
    var gainNode = null;
    var audioCtx = null;
    ...
    function initGain() {
      ...
      try {
        ...
        gainNode = null; // graceful fallback to audio.volume
      }
    }
    ...
    // Set initial display
    if (volVal) volVal.textContent = Math.round(parseFloat(volSlider.value) * 100) + "%";
    // Also set audio.volume initially (before AudioContext is set up)
    try { audio.volume = parseFloat(volSlider.value); } catch(e) {}

    // Init GainNode on first play (requires user gesture)
    audio.addEventListener("play", function() { initGain(); }, { once: false });

    // Both 'input' (desktop drag) and 'change' (iOS touch release) events
    volSlider.addEventListener("input",  applyVolume);
    volSlider.addEventListener("change", applyVolume);
  }
