(function () {
  ...
  var SEA_BASE_FRAC = window.innerWidth < 600 ? 0.38 : 0.38; // sea occupies bottom portion
  ...
  function resizeArkCanvas() {
    ...
    arkCanvas.style.width  = w + 'px';
    arkCanvas.style.height = h + 'px';
    ...
    for (var col_i = 0; col_i < cols_n2; col_i++) {
      for (var layer = 0; layer < layers2; layer += layerStep) {
        ...
        arkParticles.push({
          ...
          offsetX:   0,
          offsetY:   0,
          ph:        Math.random() * Math.PI * 2,   // independent phase for fluid motion
          phX:       Math.random() * Math.PI * 2,   // X phase (offset from Y for elliptical orbit)
          chTimer:   Math.random() * 30,             // countdown to next char swap
        });
      }
    }
  ...
  function arkLoop() {
    ...
    if (arkPhase === 'falling') {
      ...
      // ── Pass 1: compute each particle's rest position from wave ──────
      var seaDepthPx = h * SEA_BASE_FRAC;
      var SURFACE_PX = AFS * 6;
      for (var si = 0; si < n; si++) {
        var sp = arkParticles[si];
        var waveY_ = getWaveY(sp.x);
        var rd = sp.restDepth;
        var by;
        if (rd <= SURFACE_PX) {
          by = waveY_ + rd;
        } else {
          var td = (rd - SURFACE_PX) / Math.max(1, seaDepthPx - SURFACE_PX);
          td = td < 1 ? td : 1;
          by = (waveY_ + SURFACE_PX) * (1 - td) + h * td;
        }
        sp._targetY = by;
      }

      // ── Pass 2: SPH-lite pressure — nearby particles push each other ──
      // Use a spatial grid for O(n) instead of O(n²)
      var SPH_R  = AFS * 2.2;   // interaction radius
      var SPH_R2 = SPH_R * SPH_R;
      var SPH_K  = 0.18;        // pressure strength
      var CELL   = Math.ceil(SPH_R);
      var gridW  = Math.ceil(W / CELL) + 1;

      // Build grid: cell → particle indices
      var grid = {};
      for (var gi = 0; gi < n; gi++) {
        var sp2 = arkParticles[gi];
        var rx = sp2.x + sp2.offsetX;
        var ry = sp2._targetY + sp2.offsetY;
        var cx2 = Math.floor(rx / CELL);
        var cy2 = Math.floor(ry / CELL);
        var key = cx2 + ',' + cy2;
        if (!grid[key]) grid[key] = [];
        grid[key].push(gi);
      }

      // For each particle, check 3×3 neighbourhood cells
      for (var si = 0; si < n; si++) {
        var sp = arkParticles[si];
        var rx = sp.x + sp.offsetX;
        var ry = sp._targetY + sp.offsetY;
        var cx2 = Math.floor(rx / CELL);
        var cy2 = Math.floor(ry / CELL);
        var px_ = 0, py_ = 0;
        ...
        for (var ni = -1; ni <= 1; ni++) {
          for (var nj = -1; nj <= 1; nj++) {
            var nb = grid[(cx2+ni) + ',' + (cy2+nj)];
            if (!nb) continue;
            for (var nk = 0; nk < nb.length; nk++) {
              var j = nb[nk];
              if (j === si) continue;
              var sj = arkParticles[j];
              var rjx = sj.x + sj.offsetX;
              var rjy = sj._targetY + sj.offsetY;
              var ddx = rx - rjx, ddy = ry - rjy;
              var d2 = ddx*ddx + ddy*ddy;
              if (d2 < SPH_R2 && d2 > 0.01) {
                var d = Math.sqrt(d2);
                var w = 1 - d / SPH_R;       // kernel weight
                var force = w * w * SPH_K;
                px_ += (ddx / d) * force;
                py_ += (ddy / d) * force;
              }
      ...
      // ── Pass 3: cursor repulsion + decay + render ─────────────────────
      for (var si = 0; si < n; si++) {
        var sp = arkParticles[si];

        // Phase oscillation for fluid shimmer
        sp.ph  += 0.032;
        sp.phX += 0.024;
        var depthFrac0 = Math.min(1, sp.restDepth / (seaDepthPx * 0.6));
        var fluidAmp = AFS * 0.4 * (1 - depthFrac0 * 0.85);
        var fluidY = fluidAmp * Math.sin(sp.ph);
        var fluidX = fluidAmp * 0.5 * Math.sin(sp.phX);

        // Cursor repulsion
        var renderX0 = sp.x + sp.offsetX + fluidX;
        var renderY0 = sp._targetY + sp.offsetY + fluidY;
        var dy_ = renderY0 - cy_;
        var dx_ = renderX0 - cx_;
        var d2_ = dx_*dx_ + dy_*dy_;
        if (d2_ < R2 && d2_ > 0.01) {
          var d_    = Math.sqrt(d2_);
          var frac_ = 1 - d_ / R;
          var mag_  = frac_ * frac_ * R * F * 0.18;
          sp.offsetX += (dx_ / d_) * mag_;
          sp.offsetY += (dy_ / d_) * mag_;
        }

        // Gravity: pull back toward rest position
        sp.offsetX *= 0.88;
        sp.offsetY *= 0.88;

        // Clamp
        var maxOff = AFS * 3;
        if (sp.offsetX >  maxOff) sp.offsetX =  maxOff;
        if (sp.offsetX < -maxOff) sp.offsetX = -maxOff;
        if (sp.offsetY >  maxOff) sp.offsetY =  maxOff;
        if (sp.offsetY < -maxOff) sp.offsetY = -maxOff;

        // Character flicker
        sp.chTimer -= 1;
        if (sp.chTimer <= 0) {
          sp.ch = arkCharPool[Math.floor(Math.random() * arkCpLen)] || sp.ch;
          sp.chTimer = 15 + depthFrac0 * 40 + Math.random() * 10;
        }

        sp.y = sp._targetY + sp.offsetY + fluidY;
        drawArkParticle(sp, sp.x + sp.offsetX + fluidX, h);
      }
      positionArk(h);
  ...
  function positionArk(screenH) {
    ...
    if (arkCursorX > -999) {
      ...
      if (dist_cur < PUSH_R && dist_cur > 2) {
        ...
        arkBoatPushVX -= (dx_cur / dist_cur) * pushFrac * 0.5;
        arkBoatPushVY -= (dy_cur / dist_cur) * pushFrac * 0.3;
      }
    }
    // Wave disturbance → boat: sample wave slope near boat X.
    // When cursor stirs the sea nearby, the waveHeights change,
    // causing slope and curvature impulses that rock the boat.
    var wL = getWaveY(arkBoatX - 55);
    var wR = getWaveY(arkBoatX + 55);
    var wC = getWaveY(arkBoatX);
    // Horizontal: rising left side pushes boat right
    arkBoatPushVX += (wL - wR) * 0.005;
    // Vertical: wave crest under boat lifts it
    arkBoatPushVY += (wC - (wL + wR) * 0.5) * 0.010;

    // Friction
    arkBoatPushVX *= 0.93;
    arkBoatPushVY *= 0.93;
    arkBoatPushVX = Math.max(-4, Math.min(4, arkBoatPushVX));
    arkBoatPushVY = Math.max(-5, Math.min(5, arkBoatPushVY));
    ...
    arkBoatY    += (targetY - arkBoatY) * 0.10;
    arkBoatTilt += (targetTilt - arkBoatTilt) * 0.18;
    arkBoatTilt  = Math.max(-22, Math.min(22, arkBoatTilt));
