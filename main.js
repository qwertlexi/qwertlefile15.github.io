(function () {
  ...
  function drawRain() {
    ...
    octx.fillStyle = isDark ? "rgba(0,0,0,0.13)" : "rgba(0,0,0,0.06)";
  ...
  var CMD_HELP = [
    "help          — 显示指令列表",
    "Tab           — 自动补全命令",
    "<任意文字>    — AI终端对话 (signal from the void)",
    "clear         — 清空终端",
    "rain on/off   — 矩阵雨开关",
    ...
    "episode33 / aboutyou / makeok / lipstick / girlsandboys / song2 — 歌词",
    "ping <url>    — 连通性检测",
    "↑↑↓↓←→←→BA  — ???",
  ];
  ...
  /* ════════════════════════════════════════════════
     AI CHAT — unknown input → Claude API
  ════════════════════════════════════════════════ */
  var AI_BUSY = false;
  var AI_SYSTEM = "You are the terminal of a personal website called qwertlexi — a cyberpunk, matrix-rain aesthetic personal site. You speak in short, cryptic, poetic lines. Max 1-2 sentences. You can reference the matrix, signal/noise, identity, music, code, or the void. Respond in the same language the user writes in (English or Chinese). Never break character. Never say you are an AI or Claude.";

  function askAI(userText) {
    if (AI_BUSY) { termLine("signal busy — try again", "err"); return; }
    AI_BUSY = true;
    // Streaming placeholder
    var row = document.createElement("div");
    row.className = "term-line ok";
    row.innerHTML = '<span class="prompt">✓</span><span class="text ai-streaming">▌</span>';
    if (termOutput) { termOutput.appendChild(row); termOutput.scrollTop = termOutput.scrollHeight; }
    var textSpan = row.querySelector(".text");
    ...
    .then(function(data) {
      ...
      function typeChar() {
        if (i < reply.length) {
          textSpan.textContent = reply.slice(0, i+1) + "▌";
          i++;
    ...
    .catch(function() {
      textSpan.textContent = "uplink failure — ghost mode only";
      AI_BUSY = false;
    });
  }
  ...
  var ALL_CMDS = ["help","clear","rain","speed","theme","time","echo","matrix","cyber","fps","capsule","about","lyrics",
    ...
    "rain on","rain off","speed s","speed n","speed f","theme d","theme l"];
  ...
  /* ════════════════════════════════════════════════
     G · FULLSCREEN MUSIC VISUALISER
     Circular waveform / bar ring behind rain canvas
  ════════════════════════════════════════════════ */
  var vizCanvas = document.createElement("canvas");
  vizCanvas.id = "music-viz";
  vizCanvas.setAttribute("aria-hidden","true");
  vizCanvas.style.cssText = "position:fixed;inset:0;z-index:0;pointer-events:none;opacity:0;transition:opacity 1.2s";
  document.body.insertBefore(vizCanvas, document.getElementById("rain"));
  var vizCtx2 = vizCanvas.getContext("2d");
  var analyser = null, vizAniId = null, vizBars = 80;
  ...
  function initAnalyser() {
    ...
    try {
      var ac2 = getAudioCtx();
      if (!ac2) return;
      analyser = ac2.createAnalyser();
      analyser.fftSize = 256;
      var src2 = ac2.createMediaElementSource(audio);
      src2.connect(analyser);
      analyser.connect(ac2.destination);
    } catch(e) { analyser = null; }
  }

  function drawMusicViz() {
    var w = vizCanvas.width, h = vizCanvas.height;
    vizCtx2.clearRect(0, 0, w, h);
    if (!analyser) { vizAniId = requestAnimationFrame(drawMusicViz); return; }
    var buf = new Uint8Array(analyser.frequencyBinCount);
    analyser.getByteFrequencyData(buf);
    var cx2 = w/2, cy2 = h/2;
    var baseR = Math.min(w,h) * 0.22;
    var step = Math.PI * 2 / vizBars;
    for (var i = 0; i < vizBars; i++) {
      var v = buf[Math.floor(i * buf.length / vizBars)] / 255;
      var barH = baseR * 0.6 * v;
      var angle = i * step - Math.PI/2;
      var x1 = cx2 + Math.cos(angle) * baseR;
      var y1 = cy2 + Math.sin(angle) * baseR;
      var x2 = cx2 + Math.cos(angle) * (baseR + barH);
      var y2 = cy2 + Math.sin(angle) * (baseR + barH);
      var alpha = 0.12 + v * 0.45;
      vizCtx2.strokeStyle = isDark
        ? "rgba(127,217,208," + alpha + ")"
        : "rgba(1,32,40," + alpha + ")";
      vizCtx2.lineWidth = (w > 600 ? 2.5 : 1.8);
      vizCtx2.beginPath();
      vizCtx2.moveTo(x1, y1);
      vizCtx2.lineTo(x2, y2);
      vizCtx2.stroke();
    }
    // soft centre glow
    var grad = vizCtx2.createRadialGradient(cx2,cy2,0,cx2,cy2,baseR*0.9);
    grad.addColorStop(0, isDark ? "rgba(13,138,144,0.06)" : "rgba(1,32,40,0.04)");
    grad.addColorStop(1, "transparent");
    vizCtx2.fillStyle = grad;
    vizCtx2.beginPath();
    vizCtx2.arc(cx2, cy2, baseR*0.9, 0, Math.PI*2);
    vizCtx2.fill();
    vizAniId = requestAnimationFrame(drawMusicViz);
  }

  function resizeVizCanvas() {
    vizCanvas.width  = Math.round(window.innerWidth  * (window.devicePixelRatio||1));
    vizCanvas.height = Math.round(window.innerHeight * (window.devicePixelRatio||1));
  ...
  audio.addEventListener("play", function() {
    ...
    if (!vizAniId) drawMusicViz();
  });
  audio.addEventListener("pause", function() {
    vizCanvas.style.opacity = "0";
    if (vizAniId) { cancelAnimationFrame(vizAniId); vizAniId = null; }
  });
  ...
  function triggerKonami() {
    // 1. Flash message in terminal
    if (termOutput) {
      openTerminal();
      termLine("// GHOST PROTOCOL ACTIVATED — signal authenticated", "ok");
      termLine("// identity: qwertlexi · clearance: maximum · welcome back", "ok");
    }
    // 2. Rapid-fire glitch bursts
    var glitchEl = document.getElementById("glitch-overlay");
    var count = 0;
    function burstGlitch() {
      if (!glitchEl || count > 14) return;
      glitchEl.className = count % 2 === 0 ? "active-r" : "active-b";
      setTimeout(function() { glitchEl.className = ""; }, 60);
      count++;
      setTimeout(burstGlitch, 80 + Math.random()*60);
    }
    burstGlitch();
    // 3. Rain goes MAX speed for 5s
    var prevSpeed = speedIdx;
    speedIdx = 2;
    var h2 = window.innerHeight;
    cols = cols.map(function(){ return makeCol(h2, false); });
    setTimeout(function() {
      speedIdx = prevSpeed;
      cols = cols.map(function(){ return makeCol(h2, false); });
    }, 5000);
    // 4. Scramble name 3 times
    var sc = 0;
    function doKonamiScramble() {
      ...
      if (typeof scrambleOut === "function") {
        scrambleOut(function() {
          setTimeout(function() {
            scrambleIn(function() { setTimeout(doKonamiScramble, 400); });
          }, 100);
        });
      }
    }
