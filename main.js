(function () {
  ...
  function drawRain() {
    ...
    // Beat-reactive: kick = flash + trail linger + density surge
    var _energy = getBeatEnergy();
    beatEnergy += (_energy - beatEnergy) * 0.25;  // faster energy tracking
    var _delta = _energy - lastBeatEnergy;
    if (_delta > 0.08) {                           // lower threshold = catches more beats
      beatFlash = Math.min(1.0, beatFlash + _delta * 5.0); // stronger spike
    }
    lastBeatEnergy = _energy;
    beatFlash *= 0.68;  // faster decay = each beat is distinct, not smeared
  ...
  var CMD_HELP = [
    ...
    "whoami  ·  history  ·  visitors  ·  wall  ·  leave <msg>  ·  lyrics  ·  capsule",
  ...
  var ALL_CMDS = ["help","clear","rain","speed","theme","time","echo","matrix","cyber","fps","capsule","about","lyrics",
    ...
    "rain on","rain off","speed s","speed n","speed f","theme d","theme l","whoami","history","visitors","wall","leave"];
  ...
  var WALL_ISSUE  = 4;  // ← change to your issue number
