(function () {
  ...
  function openTypewriter() {
    ...
    hint.textContent = "// typewriter mode · esc to close · 15s idle → saves to capsule";
  ...
  function closeTypewriter() {
    ...
    // LTR scramble-delete: erase from left to right with glitch chars
    var txt = twText;
    var GCHARS = "▓▒░█▄▀■□◈◉⬡⬢∅∞⚡";
    var start = 0; // chars erased from the left so far
    display.style.transition = "none";

    function ltrDeleteTick() {
      if (start >= txt.length) {
        display.textContent = "▌";
        setTimeout(function() {
      ...
      // Occasionally flash a glitch char at the erase frontier
      if (Math.random() < 0.35 && start < txt.length) {
        var gc = GCHARS[Math.floor(Math.random() * GCHARS.length)];
        display.textContent = gc + txt.slice(start + 1) + "▌";
        setTimeout(function() {
          start++;
          display.textContent = txt.slice(start) + "▌";
          setTimeout(ltrDeleteTick, 18 + Math.random() * 26);
        }, 35);
      } else {
        start++;
        display.textContent = txt.slice(start) + "▌";
