(function () {
    ...
    for (var i = 0; i < cols.length; i++) {
      ...
      for (var t = 0; t < tL; t++) {
        ...
        if (t === tL - 1) {
          ...
          octx.shadowBlur = (isGhost ? 18 + ghostFade * 20 : 9) + beatFlash * 38;
        } else if (t >= tL - 4) {
          var nf = (t - (tL - 4)) / 3;
  ...
  function closeTypewriter() {
    ...
    // Find the display element (second child after hint)
    var display = ov.children[1];
    if (!display || !twText) {
      ov.style.opacity = "0";
      setTimeout(function(){ if(ov.parentNode) ov.remove(); }, 400);
      return;
    }
    // RTL scramble-delete: delete chars from right to left with glitch
    var txt = twText;
    var GCHARS = "▓▒░█▄▀■□◈◉⬡⬢∅∞⚡";
    var len = txt.length;
    var pos = len;
    ...
    function rtlDeleteTick() {
      if (pos <= 0) {
        // Final flash then remove
        display.textContent = "▌";
        setTimeout(function() {
          ov.style.transition = "opacity .35s";
          ov.style.opacity = "0";
          setTimeout(function(){ if(ov.parentNode) ov.remove(); }, 380);
        }, 120);
        return;
      }
      glitchFrames++;
      // Brief glitch char before deleting
      if (glitchFrames % 3 === 0 && pos > 0) {
        var glitchChar = GCHARS[Math.floor(Math.random() * GCHARS.length)];
        display.textContent = txt.slice(0, pos - 1) + glitchChar + "▌";
        setTimeout(function() {
          pos--;
          display.textContent = txt.slice(0, pos) + "▌";
          txt = txt.slice(0, pos);
          var delay = 18 + Math.random() * 28;
          setTimeout(rtlDeleteTick, delay);
        }, 40);
      } else {
        pos--;
        display.textContent = txt.slice(0, pos) + "▌";
  ...
  (function checkSpecialDate() {
    ...
    setTimeout(function() {
      ...
      if (pe) {
        // Remove CSS animation so JS can control opacity freely
        pe.style.animation = "none";
        pe.style.opacity = "0";
        pe.style.transition = "opacity .6s ease";
        setTimeout(function() {
          pe.textContent = match.phrase;
          pe.style.color = match.color || "var(--accent)";
          setTimeout(function() { pe.style.opacity = "0.75"; }, 50);
        }, 300);
      }
