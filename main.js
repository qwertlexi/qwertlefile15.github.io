(function () {
  ...
  var arkCursorX  = -9999;
  var arkCursorY  = -9999;
  var ARK_REPEL_R = 90;   // repulsion radius px
  var ARK_REPEL_F = 0.28; // force strength (gentle)
  ...
  function arkLoop() {
    ...
    if (arkPhase === 'falling') {
      ...
      for (var si = 0; si < arkParticles.length; si++) {
        ...
        // ── Cursor repulsion (Y-only, very gentle) ───────────────────
        var dy_ = baseY - cy_;
        var dx_ = baseX - cx_;
        var d2_ = dx_ * dx_ + dy_ * dy_;
        if (d2_ < R2 && d2_ > 0.01) {
          var d_ = Math.sqrt(d2_);
          var mag_ = (1 - d_ / R) * R * F * 0.5;
  ...
  function positionArk(screenH) {
    ...
    arkBoatPushVX = Math.max(-3, Math.min(3, arkBoatPushVX));
    arkBoatPushVY = Math.max(-4, Math.min(4, arkBoatPushVY));
    ...
    arkBoatTilt += (targetTilt - arkBoatTilt) * 0.12;
    arkBoatTilt  = Math.max(-14, Math.min(14, arkBoatTilt));
