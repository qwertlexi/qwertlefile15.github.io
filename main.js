(function () {
  ...
  function drawRain() {
    ...
    // Beat-reactive fade: strong beat = less fade (trails linger) + flash
    var _energy = getBeatEnergy();
    beatEnergy += (_energy - beatEnergy) * 0.18;
    if (_energy - lastBeatEnergy > 0.18) {
      beatFlash = 1.0; // kick detected
    }
    lastBeatEnergy = _energy;
    beatFlash *= 0.82; // decay
    var _fade = isDark
      ? (0.13 - beatEnergy * 0.07)   // trails linger on beat
      : (0.06 - beatEnergy * 0.03);
    octx.fillStyle = "rgba(0,0,0," + Math.max(0.04, _fade) + ")";
    ...
    for (var i = 0; i < cols.length; i++) {
      ...
      for (var t = 0; t < tL; t++) {
        ...
        var r, g, b, alpha;
        if (t === tL - 1) {
          var _bf = beatFlash * 0.6;
          r = col.bright ? 255 : Math.min(255, 180 + Math.round(_bf*75));
          g = 255;
          b = col.bright ? 255 : Math.min(255, 180 + Math.round(_bf*75));
          alpha = 1;
          octx.shadowColor = "rgba(100,255,140,1)";
          octx.shadowBlur = 9 + beatFlash * 22;
        } else if (t >= tL - 4) {
          var nf = (t - (tL - 4)) / 3;
  ...
  /* ════════════════════════════════════════════════
     BEAT-REACTIVE RAIN
     Analyses audio frequency data each frame.
     beatFlash  → brief brightness pulse on kick
     beatEnergy → smoothed energy drives rain glow
  ════════════════════════════════════════════════ */
  var beatAnalyser = null, beatBuf = null;
  ...
  function initBeatAnalyser() {
    ...
    try {
      ...
      var bsrc = bac.createMediaElementSource(audio);
      bsrc.connect(beatAnalyser);
      beatAnalyser.connect(bac.destination);
      beatBuf = new Uint8Array(beatAnalyser.frequencyBinCount);
    } catch(e) { beatAnalyser = null; }
  }

  function getBeatEnergy() {
    if (!beatAnalyser || !beatBuf) return 0;
    beatAnalyser.getByteFrequencyData(beatBuf);
    // sub-bass + bass bins (kick drum lives here)
    var sum = 0, n = Math.min(12, beatBuf.length);
    for (var i = 0; i < n; i++) sum += beatBuf[i];
    return sum / n / 255;
  }
  ...
  var CMD_HELP = [
    "// qwertlexi terminal — commands",
    "clear / time / fps / about",
    "rain on|off  ·  speed s|n|f  ·  theme d|l",
    "lyrics  ·  capsule  ·  ping <url>",
    "Tab to autocomplete  ·  ↑↓ history",
    "↑↑↓↓←→←→BA  — ???",
    "// anything else → signal from the void",
  ];
  ...
  function askAI(userText) {
    ...
    // ── GEMINI API ── replace YOUR_KEY with your actual key from aistudio.google.com
    var GEMINI_KEY = "AIzaSyBlOBtj1Oh12OLTJfWomOsnq6_hBnQp4_w";
    ...
    .then(function(data) {
      var reply = (data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts[0] && data.candidates[0].content.parts[0].text) || "…signal lost";
      // Typewriter effect
