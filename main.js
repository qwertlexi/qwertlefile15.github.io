(function () {
  ...
  (function () {
    ...
    setTimeout(function() {
      ...
      setTimeout(function() {
        var inner = overlay.querySelector(".boot-inner");
        if (!inner) return;
        inner.style.transition = "opacity 0.5s";
        inner.style.opacity = "0";

        setTimeout(function() {
          inner.innerHTML =
            '<div class="boot-dialogue" id="boot-dialogue"></div>' +
            '<div class="boot-enter" id="boot-enter" aria-live="polite"></div>';
          inner.style.opacity = "1";

          var dialogueEl = document.getElementById("boot-dialogue");
          var enterEl    = document.getElementById("boot-enter");

          var LINES = [
            { text: "SYSTEM BOOT COMPLETE",              delay: 0,    cls: "bd-sys"  },
            { text: "SIGNAL DETECTED · ORIGIN UNKNOWN",  delay: 600,  cls: "bd-warn" },
            { text: "TRACING SOURCE…",               delay: 1300, cls: "bd-sys"  },
            { text: "TIMESTAMP: " + (function(){
                var n=new Date();
            ...
            { text: "你不应该在这里。", delay: 3800, cls: "bd-zh"  },
            { text: "但既然来了——",        delay: 4700, cls: "bd-zh"  },
          ];
          ...
          // CTA — appears after all dialogue, touch/tap friendly
          setTimeout(function() {
            if (!enterEl) return;

            // Mobile: show tap hint; desktop: show key hint
            var isMobile = ("ontouchstart" in window) || (navigator.maxTouchPoints > 0);
            var hint = isMobile ? "触摸屏幕，进入系统" : "按任意键，进入系统";
            enterEl.innerHTML =
              '<div class="bd-cta-wrap">' +
                '<span class="bd-cta">' + hint + '</span>' +
                '<span class="bd-cursor">_</span>' +
              '</div>';
            if (isMobile) {
              // Show a tap ripple hint on mobile
              enterEl.innerHTML +=
                '<div class="bd-tap-hint">— tap anywhere —</div>';
            }

            function enterSystem(e) {
              // Prevent mis-fire during scroll on mobile
              if (e && e.type === "touchstart" && e.touches && e.touches.length > 1) return;
              document.removeEventListener("keydown", enterSystem);
              overlay.removeEventListener("click", enterSystem);
              overlay.removeEventListener("touchstart", enterSystem);
              overlay.classList.add("boot-shatter");
              setTimeout(function() { overlay.classList.add("hidden"); }, 950);

              // Auto-open terminal with welcome after entering
              setTimeout(function() { window._bootWelcomeDone = true; }, 1100);
            }

            // Delay touch listener slightly to avoid mis-fires while reading
            document.addEventListener("keydown", enterSystem);
            overlay.addEventListener("click", enterSystem);
            setTimeout(function() {
              overlay.addEventListener("touchstart", enterSystem, { passive: true });
            }, 400);
