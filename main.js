(function () {
  ...
  var SEA_BASE_FRAC = window.innerWidth < 600 ? 0.44 : 0.38; // sea occupies bottom portion
  ...
  // Finger dip state — depresses wave surface where cursor/touch is
  var fingerDipX  = -9999;  // cursor x in px (-9999 = none)
  var fingerDipAmt = 0;     // current dip depth in px (positive = pushed down)
  var fingerDipTarget = 0;

  // Pure sine-driven wave + interactive finger dip
  function stepWave(h) {
    var base     = h * (1 - SEA_BASE_FRAC);
    ...
    // Animate finger dip toward target
    fingerDipAmt += (fingerDipTarget - fingerDipAmt) * 0.18;

    for (var k = 0; k < WAVE_SAMPLES; k++) {
      var nx  = k / (WAVE_SAMPLES - 1);
      var px  = nx * w;
      var swell  = h * 0.028 * ampScale * Math.sin(nx * 6.28  - t * 0.55 + 0.0);
      var swell2 = h * 0.018 * ampScale * Math.sin(nx * 10.2  - t * 0.38 + 1.7);
      var chop   = h * 0.010 * ampScale * Math.sin(nx * 18.8  + t * 0.82 + 0.4);
      var chop2  = h * 0.006 * ampScale * Math.cos(nx * 28.0  - t * 1.10 + 2.8);
      var ripple = h * 0.003 * ampScale * Math.sin(nx * 50.0  + t * 1.65 + 1.1);

      // Finger dip: Gaussian depression centred on cursor x
      var dip = 0;
      if (fingerDipAmt > 0.5 && fingerDipX > -999) {
        var ddx   = px - fingerDipX;
        var dipR  = 160;  // dip width in px
        var gauss = Math.exp(-(ddx * ddx) / (2 * dipR * dipR));
        dip = fingerDipAmt * gauss;
      }

      waveHeights[k] = base + swell + swell2 + chop + chop2 + ripple + dip;
    }
  }
  ...
  var AFS = window.innerWidth < 600 ? 9 : 11;

  // Touch/mouse repulsion field
  var arkCursorX  = -9999;
  var arkCursorY  = -9999;
  var ARK_REPEL_R = 200;  // repulsion radius px
  var ARK_REPEL_F = 0.60; // force strength
  ...
  function arkOnPointerMove(e) {
    ...
    // Set finger dip at cursor x — deeper if touching near sea surface
    var seaSurfY = window.innerHeight * (1 - SEA_BASE_FRAC);
    if (arkCursorY > seaSurfY - 80) {
      fingerDipX      = arkCursorX;
      fingerDipTarget = Math.min(120, Math.max(30, (arkCursorY - seaSurfY + 80) * 0.9));
    } else {
      fingerDipTarget = 0;
    }
  }
  ...
  function openArk() {
    ...
    var MAX_PARTICLES = window.innerWidth < 600 ? 4000 : 7000;
    ...
    for (var col_i = 0; col_i < cols_n2; col_i++) {
      for (var layer = 0; layer < layers2; layer += layerStep) {
        ...
        var g = Math.round(210 - depthFrac * 165);
        var b = Math.round(48  - depthFrac * 28);
        var r = layer === 0 ? 50 : 0;
        ...
        arkParticles.push({
          ...
          y:         startY,
          vy:        initVy,
          vx:        0,   // no horizontal drift during fall — starts in sea phase
          r: r, g: g, b: b,
          alpha:     1,
          settled:   false,
          restDepth: restDepth,
    ...
    arkBoatX = -1; arkBoatPushVX = 0; arkBoatPushVY = 0; fingerDipAmt = 0; fingerDipTarget = 0; fingerDipX = -9999;  // reset on open
    rainEl.style.opacity = '0';
    // Shift name/tagline up so boat doesn't overlap
    var identEl = document.getElementById('ident');
    if (identEl) {
      identEl.style.transition = 'transform .7s cubic-bezier(.22,1,.36,1)';
      identEl.style.transform  = 'translate(-50%, calc(-50% - 70px))';
      setTimeout(function(){ if (identEl) identEl.style.transition = ''; }, 800);
    }
  ...
  function arkLoop() {
    ...
    if (arkPhase === 'falling') {
      ...
      var R   = ARK_REPEL_R, R2 = R * R;
      var F   = ARK_REPEL_F;
      var W   = window.innerWidth;
      var seaBase = h * (1 - SEA_BASE_FRAC);

      for (var si = 0; si < arkParticles.length; si++) {
        var sp = arkParticles[si];

        // ── X is LOCKED to original column — no horizontal drift ─────
        // Flow field was causing particles to migrate from edges to centre,
        // creating an arc shape. X stays at baseX forever.
        var baseX = sp.baseX;  // immutable column position

        // ── Y: wave surface + rest depth ────────────────────────────
        // All particles follow the wave on Y only.
        // Surface rows ride wave contour; deep rows blend toward screen bottom.
        var waveY_     = getWaveY(baseX);
        var seaDepthPx = h * SEA_BASE_FRAC;
        var SURFACE_PX = AFS * 6;  // top 6 char-rows follow wave fully

        var baseY;
        if (sp.restDepth <= SURFACE_PX) {
          baseY = waveY_ + sp.restDepth;
        } else {
          var t_depth = (sp.restDepth - SURFACE_PX) / Math.max(1, seaDepthPx - SURFACE_PX);
          t_depth = Math.min(1, t_depth);
          baseY = (waveY_ + SURFACE_PX) * (1 - t_depth) + h * t_depth;
        }

        // ── Cursor repulsion — Y-only push, X stays locked ────────
        // offsetX is NOT used: X must stay at baseX to keep sea full-width.
        // Only Y is pushed so particles rise/sink without drifting sideways.
        var dy_ = baseY - cy_;
        var dx_ = baseX - cx_;
        var d2_ = dx_ * dx_ + dy_ * dy_;
        if (d2_ < R2 && d2_ > 0.01) {
          var d_    = Math.sqrt(d2_);
          var frac_ = 1 - d_ / R;
          var mag_  = frac_ * frac_ * R * F * 1.4;
          sp.offsetY += (dy_ / d_) * mag_;
          // Surface rows: extra upward burst so they visibly scatter
          if (sp.restDepth < AFS * 4) {
            sp.offsetY -= frac_ * frac_ * 28;
          }
        }
        // No offsetX — lock X to column
        sp.offsetX = 0;
        sp.offsetY *= 0.78;

        sp.y = baseY + sp.offsetY;
        drawArkParticle(sp, baseX, h);
      }
      positionArk(h);
      ...
      if (!anyVisible) {
        ...
        if (_ident) {
          _ident.style.transition = 'transform .6s cubic-bezier(.22,1,.36,1)';
          _ident.style.transform  = '';
          setTimeout(function(){ if (_ident) _ident.style.transition = ''; }, 700);
        }
  ...
  function drawArkParticle(p, renderX, screenH) {
    ...
    // Use flat base for alpha — not the wave surface.
    // If we used wave surface, particles at wave-crests would appear bright
    // but particles at the same depth on wave-troughs would go dark/invisible,
    // causing the "squeezing to centre" look. Flat base keeps alpha uniform.
    var flatBaseY = screenH * (1 - SEA_BASE_FRAC);
    var depth = p.y - flatBaseY;
    var depthAlpha = Math.max(0.08, 1 - depth / (screenH * SEA_BASE_FRAC * 1.2));
  ...
  // Smooth boat position + tilt + touch-push interaction
  var arkBoatY      = 0;
  var arkBoatTilt   = 0;
  var arkBoatX      = -1;   // -1 = uninitialised
  // Accumulated push velocity from touch/mouse interaction
  var arkBoatPushVX = 0;   // horizontal push velocity
  var arkBoatPushVY = 0;   // vertical push velocity
  ...
  function positionArk(screenH) {
    ...
    // Initialise boat X to centre on first call
    if (arkBoatX < 0) arkBoatX = w / 2;

    // ── Touch/mouse push on boat position ──────────────────────
    // When cursor/touch is near the boat, gently push it away.
    // Slow decay so it drifts, not bounces.
    if (arkCursorX > -999) {
      var PUSH_R = 180;   // interaction radius around boat
      ...
      if (dist_cur < PUSH_R && dist_cur > 2) {
        ...
        // Push boat AWAY from cursor — gentle force
        arkBoatPushVX -= (dx_cur / dist_cur) * pushFrac * 1.8;
        arkBoatPushVY -= (dy_cur / dist_cur) * pushFrac * 1.0;
      }
    }
    // Slow friction — drifts to a stop, no spring bounce-back
    arkBoatPushVX *= 0.91;
    arkBoatPushVY *= 0.91;
    // Clamp so it doesn't fly off
    arkBoatPushVX = Math.max(-6, Math.min(6, arkBoatPushVX));
    arkBoatPushVY = Math.max(-8, Math.min(8, arkBoatPushVY));
    arkBoatX += arkBoatPushVX;

    // Keep boat on screen horizontally
    ...
    var cx   = arkBoatX;
    var span = Math.min(boatW * 0.4, w * 0.06);

    // Sample wave surface under hull (5 points)
    var y0 = getWaveY(cx - span * 2);
    var y1 = getWaveY(cx - span);
    var y2 = getWaveY(cx);
    var y3 = getWaveY(cx + span);
    var y4 = getWaveY(cx + span * 2);
    var waveTarget = y0*0.1 + y1*0.2 + y2*0.4 + y3*0.2 + y4*0.1;

    // Tilt = slope of wave under hull
    var targetTilt = Math.atan2(y4 - y0, span * 4) * 180 / Math.PI;

    // Y: follow wave + push offset, smooth with slow lerp
    var targetY = waveTarget + arkBoatPushVY * 2;
    arkBoatY    += (targetY - arkBoatY) * 0.14;
    arkBoatTilt += (targetTilt - arkBoatTilt) * 0.20;
    arkBoatTilt  = Math.max(-28, Math.min(28, arkBoatTilt));

    var clampedLeft = Math.max(halfBoat, Math.min(w - halfBoat, cx - halfBoat));
    arkSvg.style.width     = boatW + 'px';
    arkSvg.style.height    = boatH + 'px';
    arkSvg.style.top       = (arkBoatY - boatH * 0.93) + 'px';
